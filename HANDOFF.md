...
ADR 0014 before changing authentication or deployment behavior.
...
- Native hosts must use `revokeExpoSession`: revoke the exact authoritative
  ...
  `authClient.signOut` directly for host logout, and serialize sign-in/logout
  transitions so a replacement session cannot race local cleanup.
